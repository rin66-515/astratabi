...
public class DeliveryService {
    ...
    public PublicDeliveryResponse lookup(String rawToken, HttpServletRequest request) {
        ...
        if (delivery.status() == DeliveryStatus.PREPARING || !delivery.packageReady()) {
            return PublicDeliveryResponse.preparing(delivery);
        }
    ...
    @Transactional(readOnly = true)
    public void downloadFileNotImplemented(String rawTicket) {
        throw new ApiException(HttpStatus.NOT_IMPLEMENTED, "FILE_DELIVERY_NOT_IMPLEMENTED", "ファイル交付機能は現在準備中です。");
    }
    ...
    public record PublicDeliveryResponse(String state, String projectName, String recipientLabel, String deliveryNumber,
        ...
        static PublicDeliveryResponse active(PortalDelivery delivery) {
            return response("ACTIVE", delivery, "交付資料を受け取れます。");
        }
        static PublicDeliveryResponse preparing(PortalDelivery delivery) {
            return response("PREPARING", delivery, "交付資料を準備しています。お客様サポートからのご案内をお待ちください。");
        }
        static PublicDeliveryResponse expired(PortalDelivery delivery) {
            return response("EXPIRED", delivery, "この交付リンクの有効期限は終了しました。");
        }
        private static PublicDeliveryResponse response(String state, PortalDelivery delivery, String message) {
